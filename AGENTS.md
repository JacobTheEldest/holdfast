...
│   │   ├── build.yml               # Builds :stable on master
...
- **master** = Production releases ONLY. Never push directly. Builds `:stable` images.
...
- `master` - Production only. Builds `:stable` images. Never push directly.
...
- `:stable` - Latest stable release from master branch
...
8. **ALWAYS** let Release Please handle testing→master merges
9. **NEVER** push directly to master (only via Release Please)
...
| Changes not in production | Wrong workflow | Push to master (via PR) to trigger stable builds |
