...
# custom image's identity. This has been configured for the holdfast repository.
###############################################################################
...
RUN --mount=type=bind,from=ctx,source=/,target=/ctx \
    ...
    bash -c 'for script in /ctx/build/[0-9]*-*.sh; do echo "Running: ${script}" && bash "${script}"; done'
